...
namespace CDRPhotoMatchPro.Core
{
    public sealed class CorelDrawService : IDisposable
    {
        ...
        public IEnumerable<DesignRecord> ExportDesigns(string cdrPath, string cacheRoot)
        {
            ...
            try
            {
                ...
                for (int p = 1; p <= pageCount; p++)
                {
                    ...
                    // Agar grouping se kuch bhi export nahi hua, page fallback try karo
                    if (designNo == 0)
                    {
                        string baseName = SafeName(Path.GetFileNameWithoutExtension(cdrPath));
        ...
        private List<ShapeBox> GetUsableShapes(dynamic page)
        {
            ...
            for (int i = 1; i <= count; i++)
            {
                try
                {
                    ...
                    // Bahut tiny dots/stones ko ignore nahi karna,
                    // par akeli hairline/guide type cheez skip karo.
        ...
        private bool IsNear(ShapeBox a, ShapeBox b)
        {
            ...
            double avgSize =
                (Math.Max(a.Width, a.Height) + Math.Max(b.Width, b.Height)) / 2.0;
        ...
        public void Dispose()
        {
            // CorelDRAW user ka software hai.
            // Isko Quit mat karo, warna user ka opened Corel close ho sakta hai.
            try { _app = null; } catch { }
        }
    }
}
